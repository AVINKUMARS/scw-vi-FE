...
function getBlueprint(score: number | null) {
  ...
  return {
    short: 'Your founder foundation is excellent. The next challenge is scaling culture, leadership and systems.',
    plan30: [
...
export default function Founder() {
  ...
  const rawSum = Object.entries(answers).reduce(
    (acc, [, key]) => acc + (OPTION_WEIGHT[key as OptionKey] || 0),
    0,
  )
  ...
  const band = getFounderBand(isFinite(overallScore) ? overallScore : null)
  const blueprint = getBlueprint(isFinite(overallScore) ? overallScore : null)
  const hardSummary = getHardHittingSummary(
    isFinite(overallScore) ? overallScore : null,
  )
        ...
        <div className="mb-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 mb-6">
            ...
            <div className="bg-white dark:bg-neutral-800 rounded-lg p-4 border border-gray-200 dark:border-neutral-700">
              ...
              <span className={`inline-flex px-2 py-1 rounded-full text-[11px] ${band.pill}`}>
                {band.desc}
              </span>
            </div>
          </div>
                    ...
                    <div className="flex items-center justify-between pt-2 border-t border-gray-100 dark:border-neutral-700">
                      <div>
                        ...
                        <p className="text-lg font-bold text-neutral-900 dark:text-white">
                          {isFinite(sectionScore) ? sectionScore : 0}
                        </p>
                        <p className="text-[11px] text-neutral-500 dark:text-neutral-400 mt-0.5">
                          {getSectionInsight(isFinite(sectionScore) ? sectionScore : 0)}
                        </p>
                      </div>
